...
std::unique_ptr<Scene> SceneMenuMain::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
        if (mouse->isPressed(btn_exit->getRect())) {
            state.running = false;
            return nullptr;
        } else if (mouse->isPressed(btn_newgame->getRect()))
            return std::unique_ptr<Scene>(new SceneMenuTileset(graphics));
    }
    return nullptr;
}

std::unique_ptr<Scene> SceneMenuMain::reactKDown(SDL_Keycode k_code, GameState& state) {
    if (k_code == SDLK_ESCAPE) {
        state.running = false;
        return nullptr;
    }
    return nullptr;
...
std::unique_ptr<Scene> SceneMenuTileset::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
        for (int i = 0; i < tilesets; i++) {
...
std::unique_ptr<Scene> SceneMenuMode::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
        ...
        } else if (mouse->isPressed(btn_simple->getRect())) {
...
std::unique_ptr<Scene> SceneMenuDifficulty::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
        if (mouse->isPressed(btn_easy->getRect())) return std::unique_ptr<Scene>(new SceneBoard(graphics, state));
    }
    return nullptr;
}
...
std::unique_ptr<Scene> OverlayMenuQuick::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
...
std::unique_ptr<Scene> OverlayMenuQuick::reactKDown(SDL_Keycode k_code, GameState& state) {
    if (k_code == SDLK_ESCAPE){
        return std::unique_ptr<Scene>(new SceneDummy());
    }
    return nullptr;
...
void OverlaySettings::draw() {
    ...
    auto window_height = graphics->getWindowHeight();
    auto renderer = graphics->getRenderer();
    txt_texturepack->draw(window_width / 2 - txt_texturepack->getRect().w, window_height / 2 - 30, renderer);
...
std::unique_ptr<Scene> OverlaySettings::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
...
std::unique_ptr<Scene> OverlaySettings::reactKDown(SDL_Keycode k_code, GameState& state) {
    if (k_code == SDLK_ESCAPE){
        return std::unique_ptr<Scene>(new OverlayMenuQuick(graphics));
    }
    return nullptr;
...
SceneBoard::SceneBoard(Graphics* gfx, const GameState& state)
    : graphics{gfx}, spr_bg{graphics->loadSprites(std::list<std::string>{"background.jpg"}, true).front()}, board{graphics, state.tileset_chosen, state.mode_original}, overlay{nullptr} {}
...
std::unique_ptr<Scene> SceneBoard::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
        if (overlay) {
            auto overlay_new = overlay->reactMBDown(mouse, m_button, state);
            if (overlay_new) {
...
std::unique_ptr<Scene> SceneBoard::reactKDown(SDL_Keycode k_code, GameState& state) {
    if (k_code == SDLK_ESCAPE) {
        if (overlay) {
            auto overlay_new = overlay->reactKDown(k_code, state);
            if (overlay_new) {
                if (dynamic_cast<SceneDummy*>(overlay_new.get())) {
                    overlay = nullptr;
                } else {
...
std::unique_ptr<Scene> SceneWin::reactMBDown(Mouse* mouse, Uint8 m_button, GameState& state) {
    if (m_button == SDL_BUTTON_LEFT) {
        if (mouse->isPressed(btn_again->getRect()))
            return std::unique_ptr<Scene>(new SceneMenuTileset(graphics));
        else if (mouse->isPressed(btn_exit->getRect())) {
            state.running = false;
            return nullptr;
...
std::unique_ptr<Scene> SceneWin::reactKDown(SDL_Keycode k_code, GameState& state) {
    if (k_code == SDLK_ESCAPE){
        state.running = false;
        return nullptr;
    }
